...
describe('yarn.lock v4 (lockfile version 8) "real" projects', () => {
  describe.each([
    'git-remote-url',
    'goof',
    'resolutions-simple',
    ...
    test('matches expected - no pruning', async () => {
      const pkgJsonContent = readFileSync(
        join(
          __dirname,
          `./fixtures/yarn-lock-v4/real/${fixtureName}/package.json`,
        ),
        'utf8',
      );
      const yarnLockContent = readFileSync(
        join(
          __dirname,
          `./fixtures/yarn-lock-v4/real/${fixtureName}/yarn.lock`,
        ),
        'utf8',
      );
      ...
      const expectedDepGraphJson = JSON.parse(
        readFileSync(
          join(
            __dirname,
            `./fixtures/yarn-lock-v4/real/${fixtureName}/expected.json`,
          ),
          'utf8',
        ),
      );
  ...
  test('project: out-of-sync-resolutions -> throws OutOfSyncError', async () => {
    ...
    const yarnLockContent = readFileSync(
      join(__dirname, `./fixtures/yarn-lock-v4/real/${fixtureName}/yarn.lock`),
      'utf8',
    );
    const opts: YarnLockV2ProjectParseOptions = {
      includeDevDeps: false,
  ...
  it('Workspace with resolutions', async () => {
    const rootPkgJsonContent = readFileSync(
      join(
        __dirname,
        `./fixtures/yarn-lock-v4/real/resolutions-in-workspace/package.json`,
      ),
      'utf8',
    );
    const rootYarnLockContent = readFileSync(
      join(
        __dirname,
        `./fixtures/yarn-lock-v4/real/resolutions-in-workspace/yarn.lock`,
      ),
      'utf8',
    );
    const workspacePkgJsonContent = readFileSync(
      join(
        __dirname,
        `./fixtures/yarn-lock-v4/real/resolutions-in-workspace/workspace1/package.json`,
      ),
      'utf8',
    );
    ...
    const expectedDepGraphJson = JSON.parse(
      readFileSync(
        join(
          __dirname,
          `./fixtures/yarn-lock-v4/real/resolutions-in-workspace/workspace1/expected-graph.json`,
        ),
        'utf8',
      ),
    );
  ...
  describe('Workspace with multiple resolutions for the same dependency', () => {
    const fixtureBasePath = join(
      __dirname,
      './fixtures/yarn-lock-v4/real/resolutions-multiple-versions-same-dependency',
    );
...
const getHandRolledYarnLock = (fixture: string) => {
  return readFileSync(
    join(__dirname, `./fixtures/yarn-lock-v4/hand-rolled/${fixture}/yarn.lock`),
    'utf8',
  );
};
const getHandRolledPkgJson = (fixture: string) => {
  return readFileSync(
    join(
      __dirname,
      `./fixtures/yarn-lock-v4/hand-rolled/${fixture}/package.json`,
    ),
    'utf8',
  );
};

describe('yarn.lock v4 parsing', () => {
