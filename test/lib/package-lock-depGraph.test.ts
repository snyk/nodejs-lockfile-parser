...
import {buildDepGraphFromFiles} from '../../lib';
...
test('Parse npm package-lock.json', async (t) => {
  ...
  const depTree = await legacy.graphToDepTree(depGraph, 'npm');
  t.deepEqual(depTree.dependencies, expectedDepTree.dependencies, 'Tree generated as expected');
});
...
test('Parse npm package-lock.json with missing dependency', async (t) => {
    t.rejects(buildDepGraphFromFiles(
      `${__dirname}/fixtures/goof/`,
      'package.json',
...
test('Parse npm package-lock.json with missing package name', async (t) => {
  ...
  t.false(_.isEmpty(depGraph.getDepPkgs()));
  t.equals(depGraph.rootPkg.name, 'package.json');
});
...
test('Parse npm package-lock.json with empty dependencies and includeDev = false', async (t) => {
  ...
  const depTree = await legacy.graphToDepTree(depGraph, 'npm');
  t.deepEqual(depTree.dependencies, expectedDepTree.dependencies, 'Tree is created with empty deps');
});
...
test('Parse npm package-lock.json with dev deps only', async (t) => {
  ...
  const depTree = await legacy.graphToDepTree(depGraph, 'npm');
  t.deepEqual(depTree.dependencies, expectedDepTree.dependencies, 'Tree is created with dev deps only');
});
...
test('Parse npm simple package-lock.json dev and prod deps', async (t) => {
  ...
  const depTree = await legacy.graphToDepTree(depGraph, 'npm');
  t.deepEqual(depTree.dependencies, expectedDepTree.dependencies, 'Tree is created with dev and prod deps');
});
...
test('Parse npm package-lock.json with dev deps only', async (t) => {
  ...
  const depTree = await legacy.graphToDepTree(depGraph, 'npm');
  t.deepEqual(depTree.dependencies, expectedDepTreeEmpty.dependencies, 'Tree is created empty');
});
...
test('Performance: Parse big npm package-lock.json with cyclic deps and dev-deps', async (t) => {
  ...
  const depTree = await legacy.graphToDepTree(depGraph, 'npm');
  t.deepEqual(depGraph.rootPkg.name, 'trucolor', 'Tree is created correctly');
});
...
test('Out of sync project package-lock.json generates tree', async (t) => {
